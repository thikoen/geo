...
sleep 1
docker run --name wp -d -p 3000:3000 -v /postgres_db:/var/lib/postgresql/data thkoenia/wahlprojekt:05 && docker exec -d -it wp npm run seed

#starting the application
echo Starting Geo App.... Reach it under localhost:3000
sleep 5
docker exec -d -it wp npm start